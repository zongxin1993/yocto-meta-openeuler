...
do_compile:append() {
	...
	if [ "${TRUSTED_BOARD_BOOT}" = "1" ]; then
		...
		touch ${S}/build/${PLATFORM}/release/bl2-kcert.bin
		touch ${S}/build/${PLATFORM}/release/bl2.bin
		...
		mkdir -p ${S}/build/${PLATFORM}/release/
		touch ${S}/build/${PLATFORM}/release/bl31-kcert.bin
		...
		touch ${S}/build/${PLATFORM}/release/bl31.bin
		...
		if [ "${PMIC_SUPPORT}" = "1" ]; then
			sudo touch ${PMIC_BUILD_DIR}/bl2-kcert_pmic.bin
			python3 ${MANIFEST_GENERATION_KCERT} -info ${DIRPATH_MANIFEST_GENTOOL}/info/bl2_${IMG_AUTH_MODE}_info.xml \
...
do_install:append () {
	...
	if [ "${TRUSTED_BOARD_BOOT}" = "1" ]; then
		...
		mkdir -p ${D}/boot/
		install -m 0644 ${S}/build/${PLATFORM}/release/bl2.bin  ${D}/boot/bl2-${MACHINE}.bin
		...
		install -m 0644 ${S}/build/${PLATFORM}/release/bl31.bin  ${D}/boot/bl31-${MACHINE}.bin
